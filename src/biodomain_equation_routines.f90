          ...
          CASE(EQUATIONS_SET_MONODOMAIN_EQUATION_TYPE)
            ...
            CASE(EQUATIONS_SET_1D3D_MONODOMAIN_ELASTICITY_SUBTYPE,EQUATIONS_SET_MONODOMAIN_ELASTICITY_W_TITIN_SUBTYPE, &
              ...
              ELSE
                ...
                ENDIF
                ! the next check is temporarily disabled to allow different versions of the model TODO: uncomment
                !CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U2_VARIABLE_TYPE,7,ERR,ERROR,*999)
                SELECT CASE(EQUATIONS_SET%SOLUTION_METHOD)
                CASE(EQUATIONS_SET_FEM_SOLUTION_METHOD)
                  CALL FIELD_COMPONENT_INTERPOLATION_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,1, &
                    & FIELD_NODE_BASED_INTERPOLATION,ERR,ERROR,*999)
        ...
        CASE(EQUATIONS_SET_MONODOMAIN_EQUATION_TYPE)
          ...
          IF(STIFFNESS_MATRIX%UPDATE_MATRIX.OR.DAMPING_MATRIX%UPDATE_MATRIX.OR.RHS_VECTOR%UPDATE_VECTOR) THEN
            ...
            DO ng=1,QUADRATURE_SCHEME%NUMBER_OF_GAUSS
              ...
              IF (DEBUGGING) THEN
                PRINT*, "Gauss point no. ",ng,"of",QUADRATURE_SCHEME%NUMBER_OF_GAUSS
                
                !PRINT*, "   GEOMETRIC_INTERP_POINT: "
                !CALL Print_FIELD_INTERPOLATED_POINT_PTR(&
                !  & EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT(FIELD_U_VARIABLE_TYPE), 3, 3)
              
                !PRINT*, "   GEOMETRIC_INTERP_POINT_METRICS: "
                !CALL Print_FIELD_INTERPOLATED_POINT_METRICS_PTR(&
                !  & EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT_METRICS(FIELD_U_VARIABLE_TYPE), 3, 3)
              ...
              ! Interpolate geometric field
              ! interpolate value and first derivative of field at gauss point ng
              ! store in geometric_interp_point
              ...
              IF (DEBUGGING) THEN
                PRINT*, "-----------------------------"
                PRINT*, "  Gauss point no. ",ng," of ",QUADRATURE_SCHEME%NUMBER_OF_GAUSS," after field.interpolate_gauss"
                ...
                IF (.FALSE.) THEN
                  ...
                  CALL Print_FIELD_INTERPOLATED_POINT_METRICS_PTR(&
                    & EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT_METRICS(FIELD_U_VARIABLE_TYPE), 3, 3)
                ENDIF
                !PRINT*, "   GEOMETRIC_BASIS%NUMBER_OF_XI = ",GEOMETRIC_BASIS%NUMBER_OF_XI
              ENDIF
            
              !                                               JACOBIAN_TYPE,                  INTERPOLATED_POINT_METRICS
