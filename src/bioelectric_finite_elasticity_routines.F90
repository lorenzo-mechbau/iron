...
999 ERRORS("BioelectricFiniteElasticity_IterateNextMonodomainNode",ERR,ERROR)
  ...
  SUBROUTINE BioelectricFiniteElasticity_GhostElements(INDEPENDENT_FIELD_MONODOMAIN,GEOMETRIC_FIELD_MONODOMAIN,&
    ...
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    TYPE(DistributedVectorType), POINTER :: DISTRIBUTED_VECTOR
    ...
    IF (DEBUGGING.AND..FALSE.) THEN
      ...
      DO domain_idx = 1,DISTRIBUTED_VECTOR%domainMapping%NUMBER_OF_ADJACENT_DOMAINS
        DO i = 1,DISTRIBUTED_VECTOR%domainMapping%ADJACENT_DOMAINS(domain_idx)%NUMBER_OF_RECEIVE_GHOSTS
          RecvIndex = DISTRIBUTED_VECTOR%domainMapping%ADJACENT_DOMAINS(domain_idx)% &
            & LOCAL_GHOST_RECEIVE_INDICES(i)
          SendIndex = DISTRIBUTED_VECTOR%domainMapping%ADJACENT_DOMAINS(domain_idx)% &
