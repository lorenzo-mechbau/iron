...
#include "macros.h"  
  ...
  PUBLIC DOMAIN_MAPPINGS_MAPPING_FINALISE,DOMAIN_MAPPINGS_MAPPING_INITIALISE,DOMAIN_MAPPINGS_MAPPING_GLOBAL_INITIALISE, &
    ...
    ! DOMAIN_MAPPINGS_MAPPING_GLOBAL_FINALISE made public for destroying gtl maps while transitioning
CONTAINS
  ...
  SUBROUTINE DOMAIN_MAPPINGS_ADJACENT_DOMAIN_FINALISE(ADJACENT_DOMAIN,ERR,ERROR,*)
    ...
    ADJACENT_DOMAIN%NUMBER_OF_FURTHER_LINKED_GHOSTS=0
...
999 ERRORSEXITS("DOMAIN_MAPPINGS_GLOBAL_TO_LOCAL_GET",ERR,ERROR)
  ...
  SUBROUTINE DOMAIN_MAPPINGS_LOCAL_FROM_GLOBAL_CALCULATE(DOMAIN_MAPPING,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DOMAIN_MAPPING)) THEN
      ...
      ! loop over global members of the mapping
...
997   DEALLOCATE(GHOST_RECEIVE_LISTS)
  ...
  SUBROUTINE DOMAIN_MAPPINGS_MAPPING_FINALISE(DOMAIN_MAPPING,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DOMAIN_MAPPING)) THEN
      ...
      ENDIF
      IF(ALLOCATED(DOMAIN_MAPPING%domainMappingHashes)) THEN
        DO idx=1,SIZE(DOMAIN_MAPPING%domainMappingHashes,1)
          CALL DOMAIN_MAPPINGS_MAPPING_HASH_FINALISE(DOMAIN_MAPPING%domainMappingHashes(idx)%PTR,ERR,ERROR,*999)
        ENDDO !idx
        DEALLOCATE(DOMAIN_MAPPING%domainMappingHashes)
      ENDIF
...
999 ERRORSEXITS("DOMAIN_MAPPINGS_MAPPING_GLOBAL_FINALISE",ERR,ERROR)
  ...
  !> Finalises the hash mapping in the given domain mappings.
  SUBROUTINE DOMAIN_MAPPINGS_MAPPING_HASH_FINALISE(MAPPING_HASH_MAP,ERR,ERROR,*)

    !Argument variables
    TYPE(HashTableType) :: MAPPING_HASH_MAP !<The domain global mapping to finalise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<Th error string
    !Local Variables

    ENTERS("DOMAIN_MAPPINGS_MAPPING_HASH_FINALISE",ERR,ERROR,*999)

    IF(ALLOCATED(MAPPING_HASH_MAP%vecTKey)) DEALLOCATE(MAPPING_HASH_MAP%vecTKey)
    IF(ALLOCATED(MAPPING_HASH_MAP%vecTVal)) DEALLOCATE(MAPPING_HASH_MAP%vecTVal)
    IF(ALLOCATED(MAPPING_HASH_MAP%vecSKey)) DEALLOCATE(MAPPING_HASH_MAP%vecSKey)
    IF(ALLOCATED(MAPPING_HASH_MAP%arrayOfListSVal)) DEALLOCATE(MAPPING_HASH_MAP%arrayOfListSVal)

    EXITS("DOMAIN_MAPPINGS_MAPPING_HASH_FINALISE")
    RETURN
999 ERRORSEXITS("DOMAIN_MAPPINGS_MAPPING_HASH_FINALISE",ERR,ERROR)
    RETURN 1

  END SUBROUTINE DOMAIN_MAPPINGS_MAPPING_HASH_FINALISE
