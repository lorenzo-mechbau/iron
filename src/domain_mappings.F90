  ...
  SUBROUTINE DOMAIN_MAPPINGS_LOCAL_FROM_GLOBAL_CALCULATE(DOMAIN_MAPPING,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DOMAIN_MAPPING)) THEN
      ...
      DO domain_idx=1,DOMAIN_MAPPING%NUMBER_OF_ADJACENT_DOMAINS
        ...
        CALL List_RemoveDuplicatesWithoutSorting(GHOST_SEND_LISTS(domain_idx)%PTR,ERR,ERROR,*999)
...
999 ERRORSEXITS("DOMAIN_MAPPINGS_MAPPING_GLOBAL_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE DOMAIN_MAPPINGS_MAPPING_INITIALISE(workGroup,DOMAIN_MAPPING,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: NUMBER_OF_DOMAINS !<The number of domains 
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    ...
    CALL WorkGroup_NumberOfGroupNodesGet(workGroup,NUMBER_OF_DOMAINS,err,error,*999)

    DOMAIN_MAPPING%workGroup=>workGroup
